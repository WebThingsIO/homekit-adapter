...
const BLE_POLL_INTERVAL = 10 * 1000;
...
class HomeKitDevice extends Device {
  ...
  unpair() {
    return this.client.removePairing(
      ...
      console.error(`Unpairing failed for device ${this.deviceID}: ${e}`);
    });
  }
  ...
  readBLECharacteristics() {
    ...
    this.readPromise = this.client.getCharacteristics(characteristics)
      .then((result) => {
        this.handleCharacteristicEvent(result);
      })
      .catch((e) => {
        console.error(`Error reading characteristics for device ${this.deviceID
        }: ${e}`);
      }).then(() => {
        this.readPromise = null;
