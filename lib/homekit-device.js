    ...
    switch (chType) {
      ...
      case 'public.hap.characteristic.humidifier-dehumidifier.state.current': {
        ...
        property = new HomeKitProperty(
          ...
          characteristic.format,
          {
            label: 'Current Humidifier/Dehumidifier State',
            type: 'string',
            enum: Object.values(values),
            readOnly: true,
          },
      ...
      case 'public.hap.characteristic.heater-cooler.state.current': {
        ...
        property = new HomeKitProperty(
          ...
          characteristic.format,
          {
            label: 'Current Heater/Cooler State',
            type: 'string',
            enum: Object.values(values),
            readOnly: true,
          },
      ...
      case 'public.hap.characteristic.relative-humidity.dehumidifier-threshold': {
        ...
        property = new HomeKitProperty(
          ...
          {
            '@type': 'LevelProperty',
            label: 'Relative Humidity Dehumidifier Threshold',
            type: 'integer',
            unit: 'percent',
            minimum,
            maximum,
      ...
      case 'public.hap.characteristic.relative-humidity.humidifier-threshold': {
        ...
        property = new HomeKitProperty(
          ...
          {
            '@type': 'LevelProperty',
            label: 'Relative Humidity Humidifier Threshold',
            type: 'integer',
            unit: 'percent',
            minimum,
            maximum,
      ...
      case 'public.hap.characteristic.humidifier-dehumidifier.state.target': {
        ...
        property = new HomeKitProperty(
          ...
          characteristic.format,
          {
            label: 'Target Humidifier/Dehumidifier State',
            type: 'string',
            enum: Object.values(values),
          },
          characteristic.value,
      ...
      default: {
        ...
        if (c) {
          ...
          } else if (!meta.hasOwnProperty('multipleOf') &&
                     ['uint8', 'uint16', 'uint32', 'uint64'].includes(
                       characteristic.format)) {
            switch (characteristic.format) {
              case 'uint8':
              case 'uint16':
      ...
      for (const device of devices) {
        for (const property of device.properties.values()) {
          if (property.aid === characteristic.aid &&
            ...
            if (triggeredByEvent && property.label === 'Last Position State') {
